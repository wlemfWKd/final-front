...
class MBTI extends Component {
  ...
  render() {
    const { currentQuestion, showResult, mbtiType, desc, recommendedJob, recommendedLicense1, recommendedLicense2, recommendedLicense3, recommendedLicense4 } = this.state;
    return (
    <div className="Mbti">
      <div className="container text-center">
        <div className="card mt-5">
          ...
          <div className="card-body">
            {!showResult && (
              <div id="question">
                <p>{`Q${currentQuestion + 1}. ${questions[currentQuestion].question}`}</p>
                {Object.entries(questions[currentQuestion].options).map(([key, value]) => (
                  <div className="form-check" key={key}>
                    ...
                    />
                    <label className="form-check-label" htmlFor={`question${currentQuestion}${key}`}>
                      {value}
                    </label>
                  </div>
                ))}
            ...
            {showResult && (
              <div id="result">
                ...
                <h4>추천 자격증</h4>
                <p>
                  <Link to={`/detail/${recommendedLicense1}`}>{`${recommendedLicense1} `}&nbsp;&nbsp;</Link>
                  <Link to={`/detail/${recommendedLicense2}`}>{`${recommendedLicense2} `}&nbsp;&nbsp;</Link>
                  <Link to={`/detail/${recommendedLicense3}`}>{`${recommendedLicense3} `}&nbsp;&nbsp;</Link>
                  <Link to={`/detail/${recommendedLicense4}`}>{`${recommendedLicense4}`}</Link>
                </p>
                <button className="btn btn-primary mt-3" onClick={this.handleRestartTest}>
                  다시 검사하기
                </button>
              </div>
            )}
